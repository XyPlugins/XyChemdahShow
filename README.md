...
- 默认 `questhud.yml` 新增 `空任务_label`，用于单独显示无任务提示。
- 无任务时 `empty-text` 会写入 `空任务_label` 并在任务栏背景中居中显示。
- 有任务时自动隐藏 `空任务_label`，不影响正常任务标题和任务内容。
- 旧版 HUD 没有 `空任务_label` 时仍兼容原来的 `任务信息_label` 显示方式。
- 版本提升至 `1.6.6`，用于区分本次 HUD 空任务组件和统一前缀语义修正版。
...
默认 HUD 额外提供 `空任务_label`。当玩家没有正在进行的任务时，插件会把 `config.yml -> empty-text` 写入这个组件，并清空任务标题和任务内容；有任务时会自动隐藏该组件。旧 HUD 没有 `空任务_label` 时，仍会把无任务提示回退写入 `任务信息_label`。
