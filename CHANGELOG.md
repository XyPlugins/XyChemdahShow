...
- 新增多任务独立导航按钮，默认 `questhud.yml` 提供 `任务导航按钮` 与 `任务导航按钮_2` 至 `任务导航按钮_8`。
- 导航命令支持 `/xychshow nav <任务显示序号>`；HUD 点击动作分别传入对应序号。
- 点击其他任务按钮时可直接切换导航目标，点击当前任务按钮则停止导航。

### 调整

- HUD 刷新时记录任务标题行位置和任务 ID，后续按钮随对应标题及滚动位置移动。
- 第二个及之后的可导航任务标题自动缩进，为导航图标预留空间。
- 每个任务独立判断导航坐标，仅隐藏无坐标任务自己的按钮。
- 服务端收到按钮序号后使用当前 HUD 快照取得任务 ID，并再次从 Chemdah 当前任务中校验，客户端修改命令无法导航未接取任务。
- 同一玩家的 8 个按钮位置与显隐更新合并为一个 DragonCore 方法包，避免保活刷新逐按钮发包。
- 玩家退出时清理 HUD 任务快照和导航状态，避免离线玩家数据残留。

### 验证

- 已执行 `gradlew.bat clean build`，Java 编译、资源处理与 jar 打包全部通过；依赖中的 Kotlin 注解警告不影响构建。
- 已检查 jar 内版本为 `1.6.8`，默认 `questhud.yml` 包含 8 个导航按钮及第二项任务点击命令。

## 1.6.7 - 2026-08-06

### 修复

- 修复玩家挂机一段时间后 DragonCore 任务 HUD 被客户端回收，轻量组件刷新无法让任务栏重新出现的问题。

### 调整

- `hud-keep-alive.reopen-hud` 默认值改为 `true`。保活只发送 `openhud` 恢复包，不重新发送 `questhud.yml`，避免覆盖玩家已经调整好的 HUD 位置。
- 升级已有服务器时，需要手动将旧 `config.yml` 中的该配置改为 `true`，再执行 `/xychshow reload`。
