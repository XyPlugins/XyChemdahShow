...
- 玩家聊天提示前缀改为优先读取XyCore 0.3.11+ 的 `messages.prefix`。
- 保持XyCore软依赖：未安装、未启用或API不可用时，使用本插件 `messages.prefix`，不再伪装成XyCore默认前缀。
- 控制台日志继续使用XyChemdahShow自己的前缀，便于排查HUD、Chemdah事件和导航问题。
- 命令帮助中的直接聊天输出也统一走插件前缀发送入口。

### 文档

- 同步更新README、USAGE、AI使用记录、默认配置注释和版本号。
