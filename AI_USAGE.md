...
- 根据服主提供的双任务 HUD 截图，修复只有第一项任务显示导航按钮的问题。
- 让每项任务拥有独立、可校验且能随内容滚动的导航入口。

### 实现记录

- AI 辅助分析当前 HUD 使用一个固定 `任务导航按钮`、导航服务只选择首个目标的双重限制。
- HUD 刷新时记录每个任务的标题行、任务 ID 和导航可用状态，并为后续标题预留按钮宽度。
- 默认 HUD 新增第 2 至第 8 个导航按钮，按钮点击分别执行带显示序号的导航命令。
- 服务端将按钮序号映射为当前 HUD 任务 ID，再从玩家当前 Chemdah 任务中验证并读取坐标。
- 导航服务支持在不同任务之间直接切换；重复点击当前任务按钮时停止导航。
- AI 辅助将多个按钮的位置与显隐设置合并为单个 DragonCore 方法包，控制保活刷新产生的发包数量。
- 玩家退出时清理 HUD 映射与导航状态。

### 验证记录

- 已执行 `gradlew.bat clean build`，Java 编译、资源处理与 jar 打包全部通过；依赖中的 Kotlin 注解警告不影响构建。
- 已检查 jar 内 `plugin.yml` 版本为 `1.6.8`，默认 HUD 共包含 8 个任务导航按钮，并包含 `/xychshow nav 2` 点击动作。
- 运行期需在 DragonCore 客户端确认：第二个及后续按钮与标题对齐、滚动时保持同步、点击后切换到对应任务坐标。

## 2026-08-06 / 1.6.7

### 使用目标

- 根据服主反馈，修复任务栏在玩家挂机约十分钟后消失、重新进入服务器才恢复的问题。

### 实现记录

- 分析确认服务端只记录 HUD 曾经打开过，无法感知 DragonCore 客户端后续回收 HUD；原先仅刷新组件值不能恢复已消失的界面。
- 将 `hud-keep-alive.reopen-hud` 的默认值改为 `true`，保活时周期性发送 DragonCore `openhud` 恢复包。
- 保活恢复不重发 `questhud.yml`，避免因恢复操作重置玩家拖动后的 HUD 位置。

### 验证记录

- 已更新 `plugin.yml`、`build.gradle`、默认 `config.yml`、README、CHANGELOG，并准备执行 Gradle 构建验证。
- 运行期需要在实际 DragonCore 客户端挂机至少十分钟确认任务栏仍保持显示。
...
- 协助新增 HUD 保活刷新，处理 DragonCore 任务栏待久后自动消失或内容丢失的问题。
- 协助修复无任务或无导航坐标时导航按钮、地面箭头残留的问题。
- 协助同步版本号、更新记录和使用说明，便于后续发布。

### 使用方式

- AI 辅助检查 `HudService`、`NavigationService`、`PluginSettings` 与默认配置文件。
- AI 辅助新增可配置的 `hud-keep-alive` 定时器，默认仅刷新组件文本、内部变量和导航按钮显隐，不默认重发完整 HUD。
- AI 辅助将导航目标可用性判断暴露给 HUD 刷新流程，让 HUD 能根据当前任务状态隐藏或显示导航按钮。

### 验证记录

- 已进行 Java 编译验证。
- 已打包生成测试 jar。
- 运行期需在 DragonCore + Chemdah 环境中确认：任务栏长时间显示是否稳定、无任务时导航按钮是否隐藏、任务移除后地面箭头是否清理。
...
- 协助修复 XyCore 前缀使用范围不准确的问题。
- 协助将任务第一行标题从任务详情中拆出，便于导航按钮与标题单独排版。
- 协助修复导航按钮悬浮提示跑到屏幕外的问题。

### 使用方式

- 由开发者确认：玩家导航提示应使用 XyCore 前缀，后台日志、插件重载和管理命令仍应使用 `XyChemdahShow` 前缀。
- AI 辅助拆分 `log` 与 `playerLog`，导航服务改用玩家提示前缀，其他管理/后台调用保留插件前缀。
- AI 辅助新增 `任务标题_label`，HUD 服务检测到该组件后会把第一行任务标题单独写入，剩余内容写入 `任务信息_label`。
- AI 辅助在默认 `questhud.yml` 中加入固定位置的 `导航按钮提示` 文本组件，替代容易越界的原生 tip。
- AI 辅助将开发者最新提供的新版 RPG-HUD `questhud.yml` 合入默认资源，并补回标题拆分、导航按钮提示与导航视觉配置。

### 验证记录

- 已进行 Gradle 构建验证。
- 运行期需在服务器中确认：导航提示前缀、后台日志前缀、标题/详情拆分和悬浮提示位置符合实际 UI 效果。

## 2026-07-26 / 1.6.2

### 使用目标

- 协助修正导航按钮 tip 的默认配置位置。

### 使用方式

- 由开发者确认：`tip` 应写在 `任务导航按钮` 图片组件下，而不是写给独立的 `导航按钮提示` 文本组件。
- AI 辅助将默认 `questhud.yml` 的 `tip: "§e点击导航"` 放回 `任务导航按钮` 组件。
- AI 辅助移除独立 `导航按钮提示` 文本组件，避免原生 tip 与自定义提示同时存在。

### 验证记录

- 已重新构建 jar。
- 运行期需在服务器中确认：鼠标悬停 `任务导航按钮` 时显示 `点击导航`。
...
- 由开发者提出：任务导航目标在 `world`，玩家不在该世界时，需要提示玩家先传送到任务配置中的地点。
- AI 辅助在导航启动前增加目标世界检测，提示内容优先引用 `addon.xychshow.location`。
- AI 辅助处理导航过程中玩家切换世界的情况：停止本次导航并清理箭头，避免定时器空转。

### 验证记录

- 已重新构建 jar。
- 运行期需在服务器中确认：玩家不在目标世界时点击导航按钮，会收到 `请先传送到 <location> 后再开启导航`。

## 2026-07-26 / 1.6.4

### 使用目标

- 协助替换默认 `questhud.yml` 为开发者最新提供的任务栏配置。
- 协助精简默认生成文件，移除旧版 `Quest` 示例目录生成。

### 使用方式

- 由开发者提供新版 `questhud.yml`。
- AI 辅助保留 `任务导航按钮.tip` 原生悬浮提示，并移除旧 `导航按钮提示` 文本组件和相关临时变量逻辑。
- AI 辅助取消首次安装时释放 `Quest/任务配置.yml`，并调整旧任务视图目录为“存在才读取，不存在不创建”。

### 验证记录

- 已重新构建 jar。
- 已检查输出 jar 不再包含 `Quest/任务配置.yml`。
